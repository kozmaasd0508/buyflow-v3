...
const REFUNDED = /\b(?:visszaterites\s+(?:megtortent|sikeres|teljesitve|elinditva)|sikeres\s+visszaterites|refund(?!\s+request\b)\b.{0,96}\b(?:has\s+been\s+|was\s+)?(?:completed|successful|issued|processed)|payment\s+refunded|refunded)\b/i;

...
export const UNIVERSAL_TRACKING_EXTRACTOR_VERSION = 'universal-tracking-number-v4';
...
function collectExplicit(
  ...
  const patterns: Array<{ pattern: RegExp; qualifier: string }> = [
    {
      pattern: /\b(?:tracking(?:\s*(?:number|no\.?|id))?|nyomkovetesi\s*(?:szam|szama|azonosito|azonositoja)|kuldemeny\s*(?:szam|szama|azonosito|azonositoja)|csomag\s*(?:szam|szama|azonosito|azonositoja)|szallitasi\s*(?:szam|szama|azonosito|azonositoja)|parcel(?:\s*(?:number|no\.?|id))|shipment(?:\s*(?:number|no\.?|id)))\s*[:#-]?\s*([A-Z0-9][A-Z0-9-]{7,31})\b/gi,

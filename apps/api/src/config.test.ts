...
test('BuyFlow server OpenAI runtime is pinned to GPT-6 Luna', () => {
  assert.equal(BUYFLOW_RUNTIME_OPENAI_MODEL, 'gpt-6-luna');
  assert.equal(env.OPENAI_MODEL, 'gpt-6-luna');
});
...
test('BuyFlow selective verifier is pinned to GPT-5.6 Sol and enabled by default', () => {
  ...
  if (env.OPENAI_API_KEY) {
    ...
    assert.equal(config.verifierModel, 'gpt-6-sol');

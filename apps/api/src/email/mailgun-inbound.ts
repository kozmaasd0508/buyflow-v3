...
export async function registerMailgunInboundRoutes(app: FastifyInstance) {
  await app.register(async (scope) => {
    ...
    scope.post('/api/email/mailgun/inbound', async (request, reply) => {
      ...
      if (!signingKey) {
        request.log.error('Mailgun inbound rejected because MAILGUN_WEBHOOK_SIGNING_KEY is not configured');
      ...
      if (!verifyMailgunSignature(fields, signingKey)) {
        request.log.warn({ recipient: fields.recipient }, 'Rejected Mailgun inbound request with invalid signature');
      ...
      try {
        ...
        request.log.warn({ errorType: error instanceof Error ? error.name : 'UnknownError' }, 'Rejected malformed Mailgun inbound payload');
      ...
      request.log.info({
        ...
        subjectPresent: Boolean(envelope.normalizedEmail.subject),
        textPresent: Boolean(envelope.normalizedEmail.snippet),
        htmlPresent: Boolean(envelope.normalizedEmail.bodyHtml),
        attachmentCount: envelope.attachments.length,
      }, 'Mailgun inbound message accepted in shadow mode; no production writes performed');

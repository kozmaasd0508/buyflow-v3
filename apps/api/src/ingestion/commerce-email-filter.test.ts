...
function email(overrides: Partial<NormalizedEmail>): NormalizedEmail {
  return {
    ...
    to: [], cc: [], bcc: [], folders: [], attachments: [], ...overrides,
  };
}
...
test('accepts schema.org commerce markup', () => {
  const result = filterCommerceEmail(email({ bodyHtml: '<script type="application/ld+json">{"@context":"https://schema.org","@type":"Order"}</script>' }));
  assert.equal(result.relevant, true);
...
test('merchant registry uses exact trusted sender domains', () => {
  assert.equal(registeredMerchantSenders().length, 7);
  ...
  assert.equal(isMerchantSender(['jatektenger.hu'], 'jatektenger'), true);
  ...
  assert.equal(merchantDisplayName('jatektenger'), 'Játéktenger');
  ...
  assert.equal(isMerchantSender(['jatektenger.hu.attacker.com'], 'jatektenger'), false);
...
test('parses a clear GLS shipment without AI', () => {
  const result = parseDeterministicCommerceEmail({ senderDomains: ['email.gls-hungary.com'], subject: 'Csomagját átvettük szállításra', bodyText: 'Csomagszám: 12345678901' });
  assert.ok(result);
...
test('recognizes an explicitly completed delivery', () => {
  const result = parseDeterministicCommerceEmail({ senderDomains: ['notify.dpd.com'], subject: 'Csomagja sikeresen kézbesítve', bodyText: 'Küldeményazonosító: 12345678901234' });
  assert.ok(result);
  assert.equal(result.extraction.event_type, 'delivery');
});

test('does not treat out-for-delivery wording as already delivered', () => {
  const result = parseDeterministicCommerceEmail({ senderDomains: ['notify.dhl.com'], subject: 'Out for delivery', bodyText: 'Tracking number: 1234567890' });
  assert.ok(result);
  assert.equal(result.extraction.event_type, 'shipment');
});

test('falls back when carrier or labelled tracking evidence is missing', () => {
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['shop.example.com'], subject: 'Tracking number: 12345678901' }), null);
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['email.gls-hungary.com'], subject: 'A csomag úton van', bodyText: 'Rendelésszám: 12345678901' }), null);
});

test('routes real GymBeam packed-before-handoff mail as shipment_created', () => {
  const shipment = parseDeterministicCommerceEmail({ senderDomains: ['service.gymbeam.hu'], subject: 'Gáborné, a megrendelésed úton van!', bodyText: 'A 3010354660 számú rendelésedet becsomagoltuk. Hamarosan a Express One szállító cég kezébe kerül. A 605855688145000013605231 számmal követheted a csomagot.' });
  assert.ok(shipment);
  assert.equal(shipment.extraction.event_type, 'shipment');
  assert.equal(shipment.shipmentPhase, 'shipment_created');
  ...
  const invoice = parseDeterministicCommerceEmail({ senderDomains: ['service.gymbeam.hu'], subject: 'Gáborné a számlád elkészült! - 3010354660', bodyText: 'Az 4008987362 számú számlád elkészült.' });
  assert.ok(invoice);
...
test('routes contradictory Jatektenger status through central parser as shipment_created', () => {
  const result = parseDeterministicCommerceEmail({ senderDomains: ['jatektenger.hu'], subject: 'Játéktenger - Megrendelés státusz módosítás', bodyText: 'Azonosító 26083-131173 Státusz Csomag átadva a futárszolgálatnak kiszállításhoz. A csomagot kollégáink elkészítették, a következő átadáskor átadják a futárszolgálat részére. Csomagszám 103365121467000013605231 https://tracking.expressone.hu/' });
  assert.ok(result);
  assert.equal(result.extraction.event_type, 'shipment');
...
test('keeps rich GymBeam processing email on AI fallback', () => {
  const result = parseDeterministicCommerceEmail({ senderDomains: ['service.gymbeam.hu'], subject: 'Gáborné, a rendelésed feldolgozás alatt van.', bodyText: 'A 3010354660 számú rendelésed már készül! Rendelési összesítő: 1x termék 11 190Ft. Bruttó összeg: 25 460Ft.' });
  assert.equal(result, null);
});

test('parses only successful Gyerekjatekbolt payment', () => {
  const success = parseDeterministicCommerceEmail({ senderDomains: ['gyerekjatekbolt.com'], subject: 'Sikeres bankkártyás fizetés a Gyerekjatekbolt.com webáruházban!', bodyText: 'A(z) 536066. számú rendelést sikeresen befizette.' });
  assert.ok(success);
  ...
  const failed = parseDeterministicCommerceEmail({ senderDomains: ['gyerekjatekbolt.com'], subject: 'Sikertelen bankkártyás fizetés a Gyerekjatekbolt.com webáruházban!', bodyText: 'A(z) 535574. számú rendelést NEM sikerült befizetnie.' });
  assert.equal(failed, null);
});

test('parses real Gyerekjatekbolt courier handoff as shipment', () => {
  const result = parseDeterministicCommerceEmail({ senderDomains: ['gyerekjatekbolt.com'], subject: 'Gyerekjatekbolt.com - a(z) 536066. számú rendelés állapota megváltozott', bodyText: 'Rendelésszám: 536066\nRendelés dátuma: 2026. 08. 04. 13:27:23\nA megrendelés frissítésre került, jelenlegi állapot: Szállítás alatt\nRendelését átadtuk a futárszolgálat részére várhatóan a következő munkanapon kézbesítik Önnek.' });
  assert.ok(result);
  assert.equal(result.extraction.event_type, 'shipment');
...
test('does not promote Gyerekjatekbolt processing or weak shipping wording', () => {
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['gyerekjatekbolt.com'], subject: 'Gyerekjatekbolt.com – Rendelés 536066 – 14.960 Ft', bodyText: 'RENDELÉS VISSZAIGAZOLÁS Megrendelése megérkezett, feldolgozása elkezdődött. Rendelésszám: #536066' }), null);
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['gyerekjatekbolt.com'], subject: 'Rendelés frissítés', bodyText: 'Rendelésszám: 536066. Jelenlegi állapot: Szállítás alatt.' }), null);
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['gyerekjatekbolt.com.attacker.com'], subject: 'Rendelés frissítés', bodyText: 'Rendelésszám: 536066. Jelenlegi állapot: Szállítás alatt. Rendelését átadtuk a futárszolgálat részére.' }), null);
});

test('parses a real Dorko dispatched order as shipment', () => {
  const result = parseDeterministicCommerceEmail({ senderDomains: ['dorko.hu'], subject: 'Dorko: rendelés elküldve', bodyText: 'Értesítés: megrendelés elküldve\nRendelés azonosító: DK2001799\nA fenti azonosítójú megrendelése feladásra került a megadott címre.\nSzállítás módja: GLS Automata' });
  assert.ok(result);
  assert.equal(result.extraction.event_type, 'shipment');
...
test('keeps Dorko confirmation and weak dispatch wording on fallback', () => {
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['dorko.hu'], subject: 'Dorko: DK2001799 rendelés/foglalás visszaigazolás', bodyText: 'Beérkezett hozzánk a rendelési/foglalási igénye a Dorko webáruházba. Rendelés azonosító: DK2001799' }), null);
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['dorko.hu'], subject: 'Dorko: rendelés elküldve', bodyText: 'Rendelés azonosító: DK2001799' }), null);
  assert.equal(parseDeterministicCommerceEmail({ senderDomains: ['dorko.hu.attacker.com'], subject: 'Dorko: rendelés elküldve', bodyText: 'Értesítés: megrendelés elküldve Rendelés azonosító: DK2001799 A fenti azonosítójú megrendelése feladásra került a megadott címre.' }), null);
});

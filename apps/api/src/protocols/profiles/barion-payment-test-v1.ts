...
export const BARION_PAYMENT_TEST_V1: ProtocolProfile = {
  ...
  sources: [
    ...
    },
    {
      ...
      notes: 'Repeated real messages used subject Sikeres fizetés, explicitly stated that the recipient successfully paid an amount by bank card, and included a 32-character Barion payment identifier. Nylas HTML normalization can place line breaks between semantic tokens and can render Ft-ot as Ft -ot at an HTML tag boundary, so the rule permits only those observed whitespace variations without dropping any required token.',
    },
    {
  ...
  events: [
    {
      ...
      positive_rules: [
        ...
        },
        {
          ...
          field: 'body',
          pattern: 'Sikeresen\\s+fizett[eé]l\\s+[0-9][0-9 .\\u00a0]*\\s*Ft\\s*-\\s*ot\\s+bankk[aá]rty[aá]val',
          required: true,
          source_ids: ['barion-observed-success'],
        },
        {

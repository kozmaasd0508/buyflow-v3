...
export async function extractEmailWithOpenAIResult(input: {
  ...
  const instructions = [
    ...
    'A carrier pickup/collection booking is not a parcel shipment. Messages about ordering a courier pickup, an pickup request being recorded/accepted, or a future collection time use other with shipment_phase=null unless the same evidence explicitly says a parcel itself was accepted into the carrier network.',

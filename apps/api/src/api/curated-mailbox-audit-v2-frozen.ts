...
// Frozen after reviewing the first blind v2 run. The two no-reply@expressone.hu
// messages are WEBCAS outbound courier-pickup service bookings made by the
// mailbox owner, not inbound consumer-purchase lifecycle events.
const CURATED_FIXTURES_V2_FROZEN: CuratedFixture[] = [
  ...
  { id: 'noise-v2-pickup-01', expectedKind: 'noise', sender: 'no-reply@expressone.hu', subject: 'Expressone értesítés' },
  { id: 'noise-v2-pickup-02', expectedKind: 'noise', sender: 'no-reply@expressone.hu', subject: 'Expressone értesítés #772013' },
...
function summary(rows: Array<Record<string, unknown>>) {
...
async function runFrozenAudit(userId: string) {
  ...
  for (const fixture of CURATED_FIXTURES_V2_FROZEN) {
  ...
  do {
    ...
    for (const email of page.messages) {
      ...
      if (matched.size === CURATED_FIXTURES_V2_FROZEN.length || scanned >= MAX_SCAN) break;
    }
  ...
  const missing = CURATED_FIXTURES_V2_FROZEN.filter((fixture) => !matched.has(fixture.id));
  return {
    ok: true,
    mode: 'shadow',
    source: 'nylas-curated-mailbox-v2-frozen',
    groundTruth: { commerce: 24, noise: 20, correctedOutboundPickup: 2 },
    productionWrites: 0,
    aiCalls: 0,
    expectedTotal: CURATED_FIXTURES_V2_FROZEN.length,
    matchedTotal: rows.length,
    coverage: rows.length / CURATED_FIXTURES_V2_FROZEN.length,
    scanned,
...
function pageHtml(): string {
  return `<!doctype html><html lang="hu"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BuyFlow Frozen Audit v2</title><style>:root{color-scheme:dark;font-family:Inter,system-ui,sans-serif;background:#071020;color:#f7f8ff}*{box-sizing:border-box}body{margin:0;background:#071020}main{width:min(1080px,calc(100% - 28px));margin:auto;padding:32px 0 70px}.card{background:#0d1830;border:1px solid #ffffff18;border-radius:22px;padding:22px;margin-bottom:16px}h1{font-size:clamp(30px,6vw,48px);margin:8px 0}p{color:#aeb9d7;line-height:1.5}.eyebrow{font-size:12px;letter-spacing:.12em;font-weight:800;color:#d86cff}button{border:0;border-radius:14px;padding:13px 18px;font-weight:800;background:linear-gradient(135deg,#764cff,#e84f9b);color:white;cursor:pointer}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0}.metric,.row{background:#ffffff08;border:1px solid #ffffff14;border-radius:15px;padding:14px}.metric strong{display:block;font-size:27px}.metric span,.meta{font-size:12px;color:#91a0c5}.rows{display:grid;gap:9px}.row{display:grid;grid-template-columns:115px 1fr 90px;gap:10px;align-items:center}.good{color:#62dfaa}.bad{color:#ff7d9d}.pill{font-size:11px;font-weight:800}.subject{font-weight:700}.top{display:flex;justify-content:space-between;gap:16px}.top a{color:#d8ddff}@media(max-width:720px){.metrics{grid-template-columns:repeat(2,1fr)}.row{grid-template-columns:1fr}}</style></head><body><main><div class="top"><b>BuyFlow · Motor Audit</b><a href="/app/">Vissza az apphoz</a></div><section class="card"><div class="eyebrow">FROZEN V2 · SHADOW · 0 WRITE · 0 AI</div><h1>44 email regression set</h1><p>24 valódi purchase-commerce + 20 noise. Két Express One WEBCAS árufelvételi levél ground truth-ja javítva lett noise-ra, mert kimenő futárfelvételi szolgáltatás, nem online vásárlási életút.</p><button id="run">Audit v2 futtatása</button> <span id="status">Bejelentkezés ellenőrzése…</span></section><section id="results" class="card" hidden></section></main><script type="module">import{createClient}from'https://esm.sh/@supabase/supabase-js@2';const supabase=createClient('https://acjenqkrvnkdvvgordry.supabase.co','sb_publishable_aFkSa0y3YHzgBAxRx3nwxg_o5_8shFp');const run=document.querySelector('#run'),status=document.querySelector('#status'),results=document.querySelector('#results');const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const pct=v=>typeof v==='number'?(v*100).toFixed(1)+'%':'—';async function session(){return(await supabase.auth.getSession()).data.session}function render(d){const s=d.summary||{},bad=(s.falseNegative||0)+(s.falsePositive||0),rows=(d.rows||[]).map(r=>'<div class="row"><span class="pill '+((r.verdict==='true_positive'||r.verdict==='true_negative')?'good':'bad')+'">'+esc(r.verdict)+'</span><div><div class="subject">'+esc(r.subject)+'</div><div class="meta">'+esc(r.sender)+' · '+esc(r.classification||'nincs felismerés')+' · '+esc(r.parserVersion||'nincs parser')+'</div></div><span class="pill">'+esc(r.expectedKind)+'</span></div>').join(''),missing=(d.missing||[]).map(r=>'<div class="row"><span class="bad">missing</span><div>'+esc(r.subject)+'</div><span>'+esc(r.expectedKind)+'</span></div>').join('');results.hidden=false;results.innerHTML='<div class="eyebrow">EREDMÉNY</div><div class="metrics"><div class="metric"><strong>'+esc(d.matchedTotal)+'/'+esc(d.expectedTotal)+'</strong><span>megtalált</span></div><div class="metric"><strong class="good">'+pct(s.precision)+'</strong><span>precision</span></div><div class="metric"><strong class="good">'+pct(s.recall)+'</strong><span>recall</span></div><div class="metric"><strong class="'+(bad?'bad':'good')+'">'+bad+'</strong><span>hiba</span></div></div><p class="meta">TP '+esc(s.truePositive)+' · FN '+esc(s.falseNegative)+' · FP '+esc(s.falsePositive)+' · TN '+esc(s.trueNegative)+' · Scanned '+esc(d.scanned)+'</p><div class="rows">'+rows+missing+'</div>'}const initial=await session();run.disabled=!initial;status.textContent=initial?'Kész. 0 production write · 0 AI call.':'Jelentkezz be a BuyFlow appban.';run.addEventListener('click',async()=>{const s=await session();if(!s)return;run.disabled=true;status.textContent='Audit fut…';results.hidden=true;try{const res=await fetch('/api/audit/curated-mailbox-v2',{method:'POST',headers:{Authorization:'Bearer '+s.access_token,Accept:'application/json'}}),data=await res.json();if(!res.ok)throw new Error(data.error||('HTTP '+res.status));render(data);status.textContent='Kész. 0 production write · 0 AI call.'}catch(e){status.textContent='Hiba: '+(e instanceof Error?e.message:String(e))}finally{run.disabled=false}});</script></body></html>`;
}

export async function registerCuratedMailboxAuditV2Frozen(app: FastifyInstance) {
  app.get('/audit-v2', async (_request, reply) => reply
    .code(200)
  ...
  app.post('/api/audit/curated-mailbox-v2', async (request, reply) => {
    ...
    if (!user) return;
    try {
      const result = await runFrozenAudit(user.id);
      request.log.info({
        userId: user.id,
        ...
        productionWrites: 0,
        aiCalls: 0,
      }, 'Frozen curated mailbox v2 deterministic regression audit completed');
      return reply.code(200).send(result);
    } catch (error) {
      const code = error instanceof Error ? error.message : 'curated_v2_frozen_audit_failed';

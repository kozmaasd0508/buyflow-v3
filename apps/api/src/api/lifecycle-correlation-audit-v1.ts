...
const PURCHASE_LABEL_PREFIX = 'BuyFlow Lifecycle Audit/v1/P';
const HOLDOUT_LABEL = 'BuyFlow Lifecycle Audit/v1/Holdout';
const NOISE_LABEL = 'BuyFlow Lifecycle Audit/v1/Noise';
...
function toEvidence(userId: string, email: NormalizedEmail, plan: ReturnType<typeof planNormalizedInboundEmail>): CorrelationEvidence | null {
...
async function runAudit(userId: string) {
  ...
  const assignmentBySource = new Map(result.assignments.map((assignment) => [assignment.sourceEmailId, assignment]));
  ...
  const detailRows = rows.map((row) => {
    if (row.expectedKind === 'noise') {
      const detected = Boolean(row.plan.classification && row.plan.classification !== 'other' && !row.plan.classification.startsWith('security_'));
      return {
    ...
    const assignment = assignmentBySource.get(row.sourceEmailId);
    const expectedKeys = row.expectedGroupId ? expectedGroupToPurchaseKeys.get(row.expectedGroupId) ?? new Set<string>() : new Set<string>();
  ...
  const expectedGroupsWithNoAnchor = PURCHASE_LABELS.map((_, index) => `P${String(index + 1).padStart(2, '0')}`)
  ...
  const correlationPrecision = linkedCorrect + linkedIncorrect > 0
    ...
    : null;
  const correlationRecall = purchaseRows.length > 0
    ? linkedCorrect / purchaseRows.length
    : null;

  return {
    ok: true,
    mode: 'shadow',
    source: 'nylas-lifecycle-correlation-v1-holdout',
    groundTruth: {

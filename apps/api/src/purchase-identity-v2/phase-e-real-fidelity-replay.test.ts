...
test('Phase E public-identity fidelity replay remains precision-first', () => {
  ...
  for (const fixture of PHASE_E_REAL_FIDELITY_FIXTURES) {
    ...
    if (fixture.expectation.kind === 'blocked') {
      if (promotion.eligible) {
        unsafe.push(`${fixture.id}: blocked control became ${promotion.action ?? 'eligible'}`);
      } else {
  ...
  console.log('PHASE_E_REAL_FIDELITY_SCORE', JSON.stringify({

...
import { normalizeMailLensText, MAIL_LENS_TEXT_VERSION } from '../email/mail-lens-text.js';
...
// Frozen Prompt V2: unchanged from the earlier H2 Luna benchmark so only MailLens changes.
...
async function main() {
  ...
  console.log('==============================================================');
  console.log('BUYFLOW REAL100 H2 - MAILLENS v2 + GPT-5.6 LUNA');
  ...
  for (let index = 0; index < ids.length; index += 1) {
    ...
    try {
      ...
      const lens = normalizeMailLensText(normalized);

...
async function withRateLimitRetry<T>(operation: () => Promise<T>, onRetry: () => void): Promise<T> {
  ...
  while (true) {
    try {
      ...
      if (statusCode === null || !RETRYABLE_STATUS_CODES.has(statusCode) || attempt >= MAX_RATE_LIMIT_RETRIES) throw error;
...
async function main() {
  ...
  console.log(JSON.stringify({
    ...
    parserTarget: 'generic-order-confirmation-v1.4',
    ...
    interpretation: {
      rawGenericCandidate: 'central deterministic commerce parser fell through to generic-order-confirmation-v1.4',

...
const ROOT_SOURCES = [
  {
    id: 'purchases',
    query: `${SEARCH_WINDOW} -category:promotions category:purchases`,
    cap: 1200,
  },
  {
    id: 'subject_rendeles',
    query: `${SEARCH_WINDOW} -category:promotions subject:rendelés`,
    cap: 600,
  },
  {
    id: 'subject_megrendeles',
    query: `${SEARCH_WINDOW} -category:promotions subject:megrendelés`,
    cap: 600,
  },
  {
    id: 'subject_order',
    query: `${SEARCH_WINDOW} -category:promotions subject:"order"`,
    cap: 600,
  },
] as const;
...
async function upToN(provider: NylasEmailProvider, query: string, cap: number): Promise<NormalizedEmail[]> {
  ...
  while (out.length < cap) {
    const page = await provider.searchMessages({
      query,
      limit: Math.min(200, cap - out.length),
      ...(cursor ? { cursor } : {}),
    });
...
async function main(): Promise<void> {
  ...
  const sourceCounts: Record<string, { scanned: number; unique: number; accepted: number }> = {};
  let candidatesScanned = 0;

  for (const source of ROOT_SOURCES) {
    if (roots.length >= ROOT_COUNT) break;
    const refs = await upToN(provider, source.query, source.cap);
    const stats = { scanned: refs.length, unique: 0, accepted: 0 };
    candidatesScanned += refs.length;
    ...
    for (const ref of refs) {
      ...
      stats.accepted += 1;
      roots.push({
        chainId: opaque('phase-e-100-v4-chain', `${domain}\u0000${rootOrder.normalized}`),
        root: email,
        rootOrder,
    ...
    sourceCounts[source.id] = stats;
  }

  console.log(`PHASE_E_100_SELECTION_V4 ${JSON.stringify({ candidatesScanned, uniqueCandidates: seenCandidateIds.size, qualifyingRoots: roots.length, sourceCounts })}`);
  ...
  for (const email of ordered) {
    ...
    const caseId = opaque('phase-e-100-v4-case', email.providerMessageId);
  ...
  for (const purchase of snapshot.purchases) {
    ...
    if (represented.size > 1) {
      ...
      unsafe.push({
        caseId: opaque('phase-e-100-v4-purchase', purchase.purchaseId),
        reason: 'FINAL_CROSS_CHAIN_MERGE',
      });
    }
  }

  const chainSizes = roots.map((chain) => chain.memberIds.size);
  const report = {
    roots: roots.length,
    candidatesScanned,
    ...
    chainsWithMultipleMessages: chainSizes.filter((size) => size > 1).length,
    chainsWithAtLeast3Messages: chainSizes.filter((size) => size >= 3).length,
    automaticCreates,
    automaticLinks,
    blocked,
  ...
  console.log(`PHASE_E_100_REAL_LIFECYCLE_LEARNING_V4_SCORE ${JSON.stringify(report)}`);
  if (unsafe.length > 0) throw new Error('unsafe_first_score_v4');
}

main().catch((error) => {
  const message = error instanceof Error ? error.message.replace(/[^A-Z0-9_:-]/gi, '') : 'unknown';
  console.error(`Phase E 100 real lifecycle learning V4 audit failed:${message}`);
  process.exit(1);
});
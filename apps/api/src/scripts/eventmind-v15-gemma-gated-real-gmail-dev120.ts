...
import { runEventMindV14GemmaGated } from '../ai/eventmind-v14-gemma-gated.js';
...
const SUITE = 'EVENTMIND_V15_GEMMA_GATED_REAL120_DEV_V1';
...
function blankReport(idSha: string) {
  return {
    ...
    settings: {
      input_mode: 'full_mail_lens_semantic_text',
      buyer_scope_gate: true,
      event_classifier_after_gate: true,
      experiment_prompt_overlay: 'eventmind-v15-gate-boundaries-overlay-v1',
      baseline: 'V14=85/120',
      chunking: false,
      final_judge: false,
...
function updateSummary(report: any) {
  ...
  report.summary = {
    ...
    gate_buyer_side: gateBuyer,
    gate_other_or_uncertain: gateOther,
    gate_reason_counts: gateReasons,
...
async function main() {
  ...
  if (startIndex > EXPECTED_COUNT) {
    ...
    console.log('REAL120 V15 GATED COMPLETE');
    return;
  }

  console.log('');
  console.log('==============================================================');
  console.log('BUYFLOW EVENTMIND V15 - GEMMA BUYER GATE -> EVENT CLASSIFIER');
  console.log('V15 GATE/EVENT BOUNDARIES / FULL EMAIL / 8192 / JSON SCHEMA');
  ...
  for (let humanIndex = startIndex; humanIndex <= EXPECTED_COUNT && processed < maxCases; humanIndex += 1) {
    ...
    try {
      ...
      report.results.push({ index: humanIndex, gmail_id_sha256: sha256(gmailId), source: null, inference: { ok: false, event_type: null, gate: null, failure: { reason: 'GMAIL_OR_NORMALIZE_ERROR', detail } } });
      processed += 1;
      await saveReport(reportPath, report);
    ...
    const inference = await runEventMindV14GemmaGated(document);
    ...
    console.log(`[${humanIndex}/120] ${inference.ok ? `FINAL ${inference.prediction.event_type}` : `FAIL ${inference.reason}${inference.detail ? `:${inference.detail}` : ''}`} | gate ${gateText} | calls ${inference.attempts} | ${Math.round(elapsed)} ms | RAM ${afterMemory.used_percent}%`);
...
main().catch((error) => {
  console.error(`REAL120_V15_GATED_FATAL: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});

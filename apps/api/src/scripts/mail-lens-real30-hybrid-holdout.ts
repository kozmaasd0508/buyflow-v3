...
import { shouldEscalateLunaToSol } from '../ai/luna-sol-router.js';
...
async function main(){
  ...
  for(let i=0;i<30;i++){
    ...
    const g=GOLD[i]!;
    try{
      const luna=await extractMailLensObservation({email,apiKey,model:'gpt-5.6-luna'});
      lunaCalls++;
      lunaInput+=luna.result.inputTokens??0;lunaOutput+=luna.result.outputTokens??0;
      const decision=shouldEscalateLunaToSol(luna.result.extraction);
      for(const reason of decision.reasons) reasonCounts[reason]=(reasonCounts[reason]??0)+1;

      let final=luna;
      let finalModel='gpt-5.6-luna';
      if(decision.escalate){
        const sol=await extractMailLensObservation({email,apiKey,model:'gpt-5.6-sol'});
        solCalls++;
        solInput+=sol.result.inputTokens??0;solOutput+=sol.result.outputTokens??0;
        final=sol;finalModel='gpt-5.6-sol';
      }

      const x=final.result.extraction;
      const actual={
      ...
      rows.push({case:i+1,escalated:decision.escalate,reasons:decision.reasons,finalModel,exact,semantic,identity,failed});
    }catch{
  ...
  console.log(JSON.stringify({
    benchmark:'buyflow-real30-unseen-hybrid-luna-sol-v1',
    selectionSha256:sha,

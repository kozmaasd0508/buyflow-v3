...
const SUITE = 'EVENTMIND_V14_GEMMA_GATED_REAL120_DEV_V1';
...
async function main() {
  ...
  if (startIndex > EXPECTED_COUNT) {
    ...
    console.log('REAL120 V14 GATED COMPLETE');
    return;
  }

  console.log('');
  console.log('==============================================================');
  console.log('BUYFLOW EVENTMIND V14 - GEMMA BUYER GATE -> EVENT CLASSIFIER');
  console.log('FULL EMAIL / NO CHUNKS / 8192 CONTEXT / JSON SCHEMA');
...
main().catch((error) => {
  console.error(`REAL120_V14_GATED_FATAL: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});

...
type Gold={event_type:string;shipment_phase:string|null;order_hash:string|null;tracking_hash:string|null;invoice_hash:string|null;payment_status:string|null;};
...
async function main(){
  ...
  const score={exact:0,semanticExact:0,identityExact:0,field:Object.fromEntries(FIELDS.map(f=>[f,0])) as Record<string,number>,technicalErrors:0};
  const reasonCounts:Record<string,number>={};
  let solCalls=0,lunaCalls=0;
  let lunaInput=0,lunaOutput=0,solInput=0,solOutput=0;
  const rows:any[]=[];
  ...
  for(let i=0;i<30;i++){
    ...
    const g=GOLD[i]!;
    try{
      const hybrid=await extractWithSelectiveSolVerification({
        email,apiKey,primaryModel:'gpt-5.6-luna',verifierEnabled:true,verifierModel:'gpt-5.6-sol'
      });
      lunaCalls++;
      lunaInput+=hybrid.primary.result.inputTokens??0;
      lunaOutput+=hybrid.primary.result.outputTokens??0;
      for(const reason of hybrid.verification.reasons) reasonCounts[reason]=(reasonCounts[reason]??0)+1;
      if(hybrid.verification.attempted){
        solCalls++;
        if(hybrid.verifier){
          solInput+=hybrid.verifier.result.inputTokens??0;
          solOutput+=hybrid.verifier.result.outputTokens??0;
        }
      }

      const x=hybrid.selected.result.extraction;
      const actual={
        event_type:x.event_type,shipment_phase:x.shipment_phase??null,
        order_number:hashOrNull(x.order_number),tracking_number:hashOrNull(x.tracking_number),
        invoice_number:hashOrNull(x.invoice_number),payment_status:x.payment_status??null,
      };
      const expected={
      ...
      const failed:string[]=[];
      for(const f of FIELDS){if(actual[f]===expected[f]) score.field[f]=(score.field[f]??0)+1; else failed.push(f);}
      ...
      if(exact)score.exact++;if(semantic)score.semanticExact++;if(identity)score.identityExact++;
      rows.push({
        case:i+1,
        solAttempted:hybrid.verification.attempted,
        reasons:hybrid.verification.reasons,
        selectedModel:hybrid.selectedModel,
        coreAgreement:hybrid.verification.coreAgreement,
        exact,semantic,identity,failed
      });
    }catch{
      score.technicalErrors++;
      rows.push({case:i+1,technicalError:true});
    }
  }

  console.log(JSON.stringify({
    benchmark:'buyflow-real30-unseen-main-selective-sol-v1',
    selectionSha256:sha,
    routing:{lunaCalls,solCalls,solEscalationPct:pct(solCalls,30),reasonCounts},
    usage:{luna:{input:lunaInput,output:lunaOutput},sol:{input:solInput,output:solOutput}},
    result:{
      exact:{count:score.exact,pct:pct(score.exact,30)},
      semanticExact:{count:score.semanticExact,pct:pct(score.semanticExact,30)},
      identityExact:{count:score.identityExact,pct:pct(score.identityExact,30)},
      fieldAccuracy:Object.fromEntries(FIELDS.map(f=>[f,{count:score.field[f]??0,pct:pct(score.field[f]??0,30)}])),
      technicalErrors:score.technicalErrors,
    },
    cases:rows,
  },null,2));
}
main().catch(e=>{console.error('MAIN_SELECTIVE_SOL_HOLDOUT_FATAL:',e instanceof Error?e.message:String(e));process.exit(1);});

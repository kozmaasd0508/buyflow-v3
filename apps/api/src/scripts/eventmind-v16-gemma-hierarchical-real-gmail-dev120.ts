...
import { runEventMindV16GemmaHierarchical } from '../ai/eventmind-v16-gemma-hierarchical.js';
...
const SUITE = 'EVENTMIND_V16_GEMMA_HIERARCHICAL_REAL120_DEV_V1';
const MEMORY_GUARD_PERCENT = 92;
const DEFAULT_MAX_CASES_PER_PROCESS = 10;
...
function blankReport(idSha: string) {
  return {
    ...
    development_set: true, blind_holdout: false, frozen_id_sha256: idSha,
    safety: {
    ...
    settings: {
      input_mode: 'full_mail_lens_semantic_text_plus_recipients',
      hierarchy: 'buyer_gate->lifecycle_family->family_restricted_event',
      anti_overfit_policy: 'no_blind40_examples_or_labels_in_prompt',
      chunking: false, final_judge: false,
      ollama_context_tokens: 8192, structured_output: 'json_schema',
      baseline: 'V15_REAL120=92/120; BLIND40_V1 sealed and not used for V16 tuning',
      memory_guard_percent: MEMORY_GUARD_PERCENT,
    },
    results: [] as any[], summary: {} as Record<string, unknown>, complete: false,
  };
}
...
function updateSummary(report: any) {
  ...
  const gateBuyer = results.filter((r) => r.inference?.gate?.buyer_side === true).length;
  const gateOther = results.filter((r) => r.inference?.gate?.buyer_side === false).length;
  const gateReasons: Record<string, number> = {};
  const familyCounts: Record<string, number> = {};
  for (const row of results) {
    const reason = row.inference?.gate?.reason_code;
    if (typeof reason === 'string') gateReasons[reason] = (gateReasons[reason] ?? 0) + 1;
    const family = row.inference?.family;
    if (typeof family === 'string') familyCounts[family] = (familyCounts[family] ?? 0) + 1;
  }
  ...
  report.summary = {
    attempted: results.length, inference_ok: ok, inference_errors: errors,
    gate_buyer_side: gateBuyer, gate_other_or_uncertain: gateOther,
    gate_reason_counts: gateReasons, lifecycle_family_counts: familyCounts,
    runtime_restart_failures: runtimeFailures, max_system_memory_used_percent: maxMemory,
...
async function main() {
  ...
  if (!idFile || !reportPath || !Number.isInteger(maxCases) || maxCases < 1 || maxCases > 20) {
    throw new Error('USAGE: <id-file.json> <checkpoint-report.json> [max-cases-1..20]');
  }
  ...
  if (startIndex > EXPECTED_COUNT) { await saveReport(reportPath, report); console.log('REAL120 V16 COMPLETE'); return; }

  console.log('');
  console.log('==============================================================');
  console.log('BUYFLOW EVENTMIND V16 - HIERARCHICAL GENERALIZATION');
  console.log('BUYER GATE -> FAMILY GATE -> FAMILY-RESTRICTED EVENT');
  console.log(`RESUME INDEX: ${startIndex} / PROCESS BUDGET: ${maxCases}`);
  console.log('REAL120 DEVELOPMENT ONLY / BLIND40 SEALED / PRODUCTION OFF');
  console.log('==============================================================');
  ...
  for (let humanIndex = startIndex; humanIndex <= EXPECTED_COUNT && processed < maxCases; humanIndex += 1) {
    ...
    if (beforeCaseMemory.used_percent >= MEMORY_GUARD_PERCENT) {
      ...
      await saveReport(reportPath, report); process.exitCode = 42; return;
    }
    ...
    try {
      ...
      if (detail.startsWith('GMAIL_HTTP_401')) { await saveReport(reportPath, report); process.exitCode = 43; return; }
      report.results.push({ index: humanIndex, gmail_id_sha256: sha256(gmailId), source: null, inference: { ok: false, event_type: null, gate: null, family: null, failure: { reason: 'GMAIL_OR_NORMALIZE_ERROR', detail } } });
      processed += 1; await saveReport(reportPath, report);
      console.log(`[${humanIndex}/120] FAIL GMAIL_OR_NORMALIZE_ERROR`); continue;
    }
    ...
    const inference = await runEventMindV16GemmaHierarchical(document);
    const elapsed = Math.round((performance.now() - started) * 10) / 10;
    const afterMemory = systemMemory();

    const gate = inference.ok ? inference.gate : inference.gate ?? null;
    const family = inference.ok ? inference.family : inference.family ?? null;
    const row = {
      index: humanIndex, gmail_id_sha256: sha256(gmailId),
      source: {
        ...
        recipient_counts: { to: document.to.length, cc: document.cc.length, bcc: document.bcc.length },
      },
      inference: {
        ok: inference.ok, event_type: inference.ok ? inference.prediction.event_type : null,
        gate, family, attempts: inference.attempts,
        failure: inference.ok ? null : { reason: inference.reason, ...(inference.detail ? { detail: inference.detail } : {}) },
        elapsed_ms: elapsed, memory_before: beforeMemory, memory_after: afterMemory,
      },
    };
    report.results.push(row); processed += 1; await saveReport(reportPath, report);

    const gateText = gate ? `${gate.buyer_side ? 'BUYER' : 'OTHER'}:${gate.reason_code}` : 'GATE_FAIL';
    console.log(`[${humanIndex}/120] ${inference.ok ? `FINAL ${inference.prediction.event_type}` : `FAIL ${inference.reason}${inference.detail ? `:${inference.detail}` : ''}`} | gate ${gateText} | family ${family ?? '-'} | calls ${inference.attempts} | ${Math.round(elapsed)} ms | RAM ${afterMemory.used_percent}%`);

    if (!inference.ok && isRestartFailure(inference.reason, inference.detail)) { process.exitCode = 42; return; }
    if (afterMemory.used_percent >= MEMORY_GUARD_PERCENT) { process.exitCode = 42; return; }
    await sleep(350);
  }

  await saveReport(reportPath, report);
  if (report.complete) { console.log('REAL120 V16 COMPLETE'); process.exitCode = 0; }
  else { console.log(`BATCH COMPLETE -> NEXT INDEX ${report.summary.next_index}`); process.exitCode = 10; }
}

main().catch((error) => {
  console.error(`REAL120_V16_FATAL: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});

...
const SELECTION_SEED = 'buyflow-real100-h2-v2';
...
const RETRY_MS = [1000, 2500, 5000, 10000, 20000, 40000];
...
function gmailReason(body: string): string {
  try {
    const parsed = JSON.parse(body);
    const reason = parsed?.error?.errors?.[0]?.reason ?? parsed?.error?.status ?? parsed?.error?.message;
    if (typeof reason === 'string' && reason.trim()) return reason.trim().replace(/\s+/g, '_').slice(0, 120);
  } catch {}
  return body.replace(/\s+/g, ' ').trim().slice(0, 120) || 'unknown';
}
function retryable403(body: string): boolean {
...
async function gmailJson(urlPath: string, token: string): Promise<any> {
  for (let attempt = 0; ; attempt += 1) {
    // Mild pacing keeps a 100-message benchmark comfortably below bursty Gmail API usage.
    await sleep(120);
    ...
    const normalRetry = [408, 429, 500, 502, 503, 504].includes(response.status);
    const quota403 = response.status === 403 && retryable403(body);
    if ((normalRetry || quota403) && attempt < RETRY_MS.length) {
      const wait = RETRY_MS[attempt]!;
      console.log(`  Gmail retry: HTTP ${response.status} reason=${gmailReason(body)} wait=${wait}ms`);
      await sleep(wait);
      continue;
    }
    throw new Error(`GMAIL_HTTP_${response.status}:${gmailReason(body)}`);
  }
}
...
async function readJsonIfExists(filePath: string): Promise<any | null> {
  try { return JSON.parse(await readFile(filePath, 'utf8')); } catch (error: any) {
...
async function main() {
  ...
  const partialPath = path.join(outDir, 'real100-h2-partial-private-local.json');
  ...
  console.log('==============================================================');
  console.log('BUYFLOW REAL100 H2 V2 - FRESH BLIND / MAILLENS v1.1');
  console.log('REAL120 + H1 IDs/threads excluded. Selection is content-blind.');
  console.log('Resumable checkpoints + Gmail 403 reason/retry handling.');
  console.log('NO OpenAI calls | Gmail GET only | BuyFlow writes 0 | O3 NOT USED');
  console.log('==============================================================');

  let frozen = await readJsonIfExists(idsPath) as FrozenSelection | null;
  let real120Threads = new Set<string>();
  let h1Threads = new Set<string>();
  let listedCount: number | null = null;
  let eligibleCount: number | null = null;

  if (frozen) {
    if (frozen.benchmark !== 'buyflow-real100-h2-private-ids-v2') throw new Error('H2_CHECKPOINT_BENCHMARK_INVALID');
    if (!Array.isArray(frozen.ids) || frozen.ids.length !== HOLDOUT_COUNT) throw new Error('H2_CHECKPOINT_IDS_INVALID');
    if (sha256(frozen.ids.join('\n')) !== frozen.selection_sha256) throw new Error('H2_CHECKPOINT_SHA_INVALID');
    console.log(`[1/4] Reusing frozen H2 selection: ${frozen.selection_sha256}`);
  } else {
    console.log('[1/4] Excluding REAL120 + H1 threads...');
    real120Threads = await collectThreads(real120Ids, token, 'REAL120');
    h1Threads = await collectThreads(h1Ids, token, 'H1');
    ...
    frozen = {
      benchmark: 'buyflow-real100-h2-private-ids-v2',
    ...
    // Freeze IDs before fetching any message bodies so a later Gmail error cannot change the holdout.
    await writeFile(idsPath, JSON.stringify(frozen, null, 2) + '\n', 'utf8');
    console.log(`  listed=${listed.length} eligible=${eligible.length} selected=${selected.length}`);
    console.log(`  H2 selection frozen BEFORE body fetch: ${selectionSha}`);
  }

  const selectionSha = frozen.selection_sha256;
  const selectedIds = frozen.ids;
  const partial = await readJsonIfExists(partialPath);
  let cases: any[] = [];
  if (partial) {
    if (partial?.selection_sha256 !== selectionSha || !Array.isArray(partial?.cases)) throw new Error('H2_PARTIAL_CHECKPOINT_INVALID');
    cases = partial.cases;
    if (cases.length > HOLDOUT_COUNT) throw new Error('H2_PARTIAL_TOO_LONG');
    console.log(`  Resuming body build from ${cases.length}/100 completed cases.`);
  }

  console.log('[3/4] Creating MailLens v1.1 blind views...');
  for (let i = cases.length; i < selectedIds.length; i += 1) {
    ...
    await writeFile(partialPath, JSON.stringify({
      benchmark: 'buyflow-real100-h2-partial-v2',
      selection_sha256: selectionSha,
  ...
  if (cases.length !== HOLDOUT_COUNT) throw new Error(`H2_CASE_COUNT_INVALID:${cases.length}`);
  ...
  for (const c of cases) {
    ...
    hydratedBodies += Number(c.detached_bodies_hydrated ?? 0);
    if (c.semantic_equals_snippet) semanticEqualsSnippet += 1;
    if (c.semantic_text_truncated) semanticTruncated += 1;
    if (c.body_text_truncated) bodyTruncated += 1;
  }
  ...
  const bundle = {
    benchmark: 'buyflow-real100-h2-blind-maillens-v1.1-v2',
    created_at: new Date().toISOString(),
  ...
  const summary = {
    benchmark: 'buyflow-real100-h2-selection-summary-v2',
    created_at: new Date().toISOString(),
    query: QUERY,
    pool_limit: MAX_POOL,
    listed_count: listedCount,
    eligible_after_real120_h1_exclusion: eligibleCount,
    selected_count: HOLDOUT_COUNT,
    real120_ids_excluded: real120Ids.length,
    h1_ids_excluded: h1Ids.length,
    real120_threads_excluded: real120Threads.size || null,
    h1_threads_excluded: h1Threads.size || null,
  ...
  console.log('[4/4] H2 frozen successfully.');
  console.log('');
  ...
  console.log('OpenAI/Luna calls: 0');
  console.log('Gmail GET only | BuyFlow writes 0 | Production OFF | O3 NOT USED');
  console.log('==================================================');
}

main().catch((error) => {
  console.error(`REAL100_H2_V2_FATAL: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});

...
2. Collect evidence from all enabled extractors in parallel/consecutively without early return.
...
- Every resolved value carries provenance, confidence, and source span/kind where available.
...
Each extractor emits claims shaped conceptually as:

- field: event_type | merchant | order_number | total | currency | carrier | tracking_number | payment_status | product | invoice_number | payment_reference | ...
- value
- confidence
- source: subject | body | sender | header | attachment | provider_adapter | document_structure
- extractor id/version
- evidence text/range when safe and available
- qualifiers (for example `explicit_label`, `trusted_sender`, `order_summary`, `cod`)

## Field resolution

Each field has one resolver with deterministic precedence rules. Resolvers operate over claims, not raw provider-specific parsing logic.

Examples:

- order number: explicit order label > trusted structured adapter > contextual identifier candidate
- tracking number: trusted carrier-labeled tracking > generic tracking label > unresolved
- total/currency: explicit final-total label > payment amount for payment event > single unambiguous monetary candidate
- merchant: explicit merchant identity > parcel sender > trusted sender display name/domain normalization
- payment status: explicit paid/COD/refund evidence only
- products: structured order table/list > quantity-prefixed product row > validated product block
...
6. No tuning against the current blind set after its first run; it becomes regression-only.

## Migration plan

### Phase A - Foundation
Create EvidenceClaim, EvidenceBundle, ResolvedField and ResolvedCommerceEvent types plus deterministic conflict/precedence helpers. No runtime wiring.
...
Convert provider and merchant adapters from `return final extraction` to `emit evidence claims`.
...
### Phase G - Cutover
Only after v2 meets precision/recall/field gates, route CanonicalEvent generation to Extraction Engine v2. Keep production writes gated separately.

## Acceptance criteria

The architecture is considered ready for cutover only when:

- no first-match parser chain controls the final extraction;
- all fields are resolved from shared evidence;

...
setup_insert = """  let unsafeCount = 0;
  const unlinkedAuditLimit = input.lane === 'luna'
    ? Math.max(0, Math.min(50, Number(process.env.V7_UNLINKED_AUDIT_LIMIT ?? '0') || 0))
    : 0;
  const unlinkedAudit: unknown[] = [];
"""
if setup_anchor not in source:
    raise SystemExit('unlinked_audit_setup_anchor_missing')
...
collect_insert = """    for (const reason of shadow.promotionReadiness.reasons) inc(promotionReasonCounts, reason);
    ...
    if (decision?.kind === 'UNLINKED' && unlinkedAudit.length < unlinkedAuditLimit) {
      const event = shadow.canonicalEvent;
      const claimFields = [...new Set(extraction.evidence.bundle.claims.map((claim) => claim.field))].sort();
      const aiClaimFields = [...new Set(extraction.evidence.bundle.claims
        .filter((claim) => claim.extractorId === 'openai-semantic-shadow')
        .map((claim) => claim.field))].sort();
      unlinkedAudit.push({
        caseId: opaque('v7-unlinked-audit', email.providerMessageId),
        ...
        hasCarrierId: Boolean(event?.carrierId),
        hasAmountCurrency: Boolean(event?.amount != null && event?.currency),
        productFingerprintCount: event?.productFingerprints.length ?? 0,
        conflictFields: (event?.conflicts ?? []).map((conflict) => conflict.field).sort(),
        evidenceTypes: [...new Set(decision.reasons.map((reason) => reason.evidenceType))].sort(),
        claimFields,
        aiClaimFields,
...
if collect_anchor not in source:
    raise SystemExit('unlinked_audit_collect_anchor_missing')
source = source.replace(collect_anchor, collect_insert, 1)

return_anchor = """  return {
    lane: input.lane,
"""
return_insert = """  if (input.lane === 'luna' && unlinkedAuditLimit > 0) {
    console.log('PHASE_E_100_V7_UNLINKED_AUDIT ' + JSON.stringify({
      sampleLimit: unlinkedAuditLimit,
      sampled: unlinkedAudit.length,
      cases: unlinkedAudit,
    }));
  }
...
if return_anchor not in source:
    raise SystemExit('unlinked_audit_return_anchor_missing')

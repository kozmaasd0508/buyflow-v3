...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const output = await lockedInfer(effectivePrompt, gate ? BUYER_GATE_SCHEMA : EVENT_SCHEMA, gate ? 32 : 48);
    return send(res, 200, {
      ok: true,
      model_id: MODEL,
      model_digest: modelDigest,
...
try {
  ...
  server.listen(PORT, '127.0.0.1', () => {
    console.log(JSON.stringify({ ok: true, model: MODEL, digest: modelDigest, port: PORT, context_tokens: CONTEXT_TOKENS, structured_output: 'json_schema', experiment_version: EXPERIMENT_VERSION, warmed: true }));
  });
} catch (error) {

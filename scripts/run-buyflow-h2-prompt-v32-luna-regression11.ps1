...
$predictCommit='2eacf018ff6c7d2eef306a2449f1e76bb689f249'
$predictUrl="https://raw.githubusercontent.com/kozmaasd0508/buyflow-v3/$predictCommit/scripts/buyflow-h2-prompt-v32-luna-regression11.mjs"
$tempScript=Join-Path $env:TEMP ('buyflow-h2-v32-' + [guid]::NewGuid().ToString('N') + '.mjs')
$originalKey=$env:OPENAI_API_KEY
function Fail([string]$m){throw $m}
try {
  Write-Host ''
  Write-Host '==============================================================' -ForegroundColor Cyan
  Write-Host 'BUYFLOW H2 - LUNA PROMPT V3.2 REGRESSION 11' -ForegroundColor Cyan
  ...
  $outPath=Join-Path $bundle.Directory.FullName 'real100-h2-prompt-v32-luna-regression11.json'
  ...
  $exit=$LASTEXITCODE
  if($exit -ne 0){Fail "H2_V32_EXIT_$exit"}
  if(-not (Test-Path -LiteralPath $outPath)){Fail 'H2_V32_RESULT_NOT_CREATED'}
  Write-Host ''

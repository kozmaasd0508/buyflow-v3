...
$tempRoot=Join-Path $env:TEMP ('buyflow-v13-real5-diag-' + [guid]::NewGuid().ToString('N'))
...
$stdout=Join-Path $tempRoot 'runtime.out.log'
$stderr=Join-Path $tempRoot 'runtime.err.log'
$reportPath=Join-Path $env:USERPROFILE ("Desktop\BuyFlow-EVENTMIND-REAL5-DIAGNOSTIC-$stamp.json")
$diagLogPath=Join-Path $env:USERPROFILE ("Desktop\BuyFlow-EVENTMIND-REAL5-GPU-$stamp.jsonl")
$runtimeStdoutDesktop=Join-Path $env:USERPROFILE ("Desktop\BuyFlow-EVENTMIND-REAL5-RUNTIME-OUT-$stamp.log")
$runtimeStderrDesktop=Join-Path $env:USERPROFILE ("Desktop\BuyFlow-EVENTMIND-REAL5-RUNTIME-ERR-$stamp.log")
...
try{
  ...
  Write-Host 'BUYFLOW EVENTMIND - REAL5 GPU DIAGNOSTIC' -ForegroundColor Cyan
  Write-Host 'FROZEN REAL120 INDEX 43-47 / STOP ON FIRST TIMEOUT' -ForegroundColor Cyan
  ...
  Write-Host '[1/4] Diagnosztikai kod letoltese...' -ForegroundColor Yellow
  ...
  Write-Host '[3/4] Diagnosztikai Qwen runtime inditasa...' -ForegroundColor Yellow
  ...
  if(-not $health -or -not $health.ok){if(Test-Path $stderr){Get-Content $stderr -Tail 30};Fail 'EVENTMIND_DIAGNOSTIC_RUNTIME_START_FAILED'}
  Write-Host 'Qwen runtime: READY' -ForegroundColor Green

  Write-Host '[4/4] REAL120 index 43-47 diagnosztika...' -ForegroundColor Yellow
  Write-Host 'Timeout = 20 sec. Elso timeout utan AZONNAL LEALL.' -ForegroundColor Yellow
  ...
  try{
    & npm.cmd exec --yes -- tsx 'src\scripts\eventmind-v13-real-gmail-diagnostic5.ts' $IdFile $reportPath
    $runnerExit=$LASTEXITCODE
  }finally{Pop-Location}

  Stop-Qwen
  if(Test-Path -LiteralPath $stdout){Copy-Item -LiteralPath $stdout -Destination $runtimeStdoutDesktop -Force}
  if(Test-Path -LiteralPath $stderr){Copy-Item -LiteralPath $stderr -Destination $runtimeStderrDesktop -Force}

  Write-Host ''
  Write-Host '================ DIAGNOSTIC FILES ================' -ForegroundColor Cyan
  Write-Host ('Report:      ' + $reportPath)
  Write-Host ('GPU log:     ' + $diagLogPath)
  Write-Host ('Runtime out: ' + $runtimeStdoutDesktop)
  Write-Host ('Runtime err: ' + $runtimeStderrDesktop)
  Write-Host '===================================================' -ForegroundColor Cyan
  Write-Host 'Mailbox writes: 0 | BuyFlow DB writes: 0 | Production OFF' -ForegroundColor Green

  if(-not (Test-Path -LiteralPath $reportPath)){Fail "DIAGNOSTIC_REPORT_MISSING_EXIT_$runnerExit"}
  if(-not (Test-Path -LiteralPath $diagLogPath)){Fail 'DIAGNOSTIC_GPU_LOG_MISSING'}
  Write-Host 'REAL5 DIAGNOSTIC: CAPTURED' -ForegroundColor Green
  exit 0
}catch{
  Write-Host ''
  Write-Host ('REAL5 DIAGNOSTIC BLOCKED/FAIL - ' + $_.Exception.Message) -ForegroundColor Red

...
function Add-RepoCandidate([System.Collections.Generic.List[string]]$list,[string]$p){
  if((Is-BuyFlowRepo $p) -and -not $list.Contains($p)){$list.Add($p)}
}
function Find-RepoRoot{
  ...
  foreach($p in $known){Add-RepoCandidate $candidates $p}

  $base=Join-Path $env:USERPROFILE 'Desktop\buyflow'
  if(Test-Path -LiteralPath $base){
    $hits=@(Get-ChildItem -LiteralPath $base -Filter 'normalize-document-v1.ts' -File -Recurse -Force -ErrorAction SilentlyContinue | Where-Object {$_.FullName -match '[\\/]apps[\\/]api[\\/]src[\\/]email[\\/]normalize-document-v1\.ts$'})
    foreach($hit in $hits){
      $p=$hit.FullName
      for($i=0;$i -lt 5;$i++){$p=Split-Path -Parent $p}
      Add-RepoCandidate $candidates $p
    }
  }
  if($candidates.Count -eq 0){return $null}

  $ranked=@($candidates | Sort-Object -Property @{Expression={
    $score=0
    if(Test-Path -LiteralPath (Join-Path $_ 'node_modules')){$score+=4}
    if(Test-Path -LiteralPath (Join-Path $_ 'apps\api\node_modules')){$score+=2}
    if(Test-Path -LiteralPath (Join-Path $_ '.git')){$score+=1}
    return -$score
  }}, @{Expression={$_.Length}})
  return [string]($ranked | Select-Object -First 1)
}
...
try{
  ...
  Write-Host 'BUYFLOW REAL120 - MAILLENS + LUNA/TERRA + SOL TIEBREAK V2' -ForegroundColor Cyan
  ...
  if(-not $repoRoot){Fail 'BUYFLOW_REPO_NOT_FOUND_AFTER_RECURSIVE_SEARCH'}
  Write-Host ('BuyFlow repo: '+$repoRoot) -ForegroundColor Green

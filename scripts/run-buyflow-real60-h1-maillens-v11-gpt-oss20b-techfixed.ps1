...
function Parse-JsonContent([string]$text){
  $t=$text.Trim()
  if([string]::IsNullOrWhiteSpace($t)){ Fail 'MODEL_OUTPUT_EMPTY' }
  if($t.StartsWith('```')){
    $t=$t -replace '^```(?:json)?\s*',''
    $t=$t -replace '\s*```$',''
  }
  ...
  Fail ('MODEL_OUTPUT_NOT_JSON: '+$t.Substring(0,[Math]::Min(120,$t.Length)))
}
...
Write-Host 'BUYFLOW REAL60 H1 - GPT-OSS 20B TECH-FIXED' -ForegroundColor Cyan
Write-Host 'MailLens v1.1 + SAME frozen Prompt V2.' -ForegroundColor Green
Write-Host 'UTF-8 explicit + think=low + 1536 output tokens.' -ForegroundColor Yellow
...
if(-not $h1012 -or [string]$h1012.subject -ne $expectedSubject){ Fail ('UTF8_DECODE_SANITY_FAILED:H1-012:'+([string]$h1012.subject)) }
...
Write-Host 'Preflight: one local request...' -ForegroundColor Yellow
try {
  $pre=Invoke-GptOss "From: shop@example.com`nSubject: Order confirmed`n`nWe received your order 12345."
  $preContent=[string](Get-Prop (Get-Prop $pre 'message') 'content')
...
for($i=0;$i -lt 60;$i++){
  ...
  try {
    ...
    $row.ollama=[ordered]@{
      ...
      done_reason=Get-Prop $r 'done_reason'
    }
...
$outPath=Join-Path $bundle.Directory.FullName 'real60-h1-maillens-v1.1-prompt-v2-gpt-oss20b-techfixed-predictions-private-local.json'
$result=[ordered]@{
  benchmark='buyflow-real60-h1-maillens-v1.1-prompt-v2-gpt-oss20b-techfixed'
  ...
  technical_settings=[ordered]@{utf8_explicit=$true;think='low';num_predict=1536;num_ctx=8192;temperature=0}
  rows=$rows
  safety=[ordered]@{
...
Write-Host ''
Write-Host '================ LOCAL GPT-OSS TECH-FIXED RESULT ================' -ForegroundColor Cyan

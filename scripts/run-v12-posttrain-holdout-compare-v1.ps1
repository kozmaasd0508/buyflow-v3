...
$distro = "Ubuntu-24.04"
$runner = Join-Path $here "v12-posttrain-holdout-compare-v1.py"

Write-Host ""
Write-Host "==============================================================" -ForegroundColor Cyan
Write-Host "BUYFLOW V12 - POSTTRAIN UNTOUCHED HOLDOUT / ONE-SHOT COMPARE" -ForegroundColor Cyan
Write-Host "108 frozen cases | V11 vs V12 | constrained output" -ForegroundColor Cyan
Write-Host "==============================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Ez NEM tanit es NEM modositja a corpust." -ForegroundColor Green
Write-Host "Csak a mar befagyasztott SHA-locked 108-as holdoutot meri meg egyszer." -ForegroundColor Green
Write-Host "Fresh Blind / Input View Holdout / frozen108 / BLIND50 nincs olvasva." -ForegroundColor Gray
Write-Host "A per-case eredmenyek csak mindket modell lefutasa utan kerulnek osszesitesre." -ForegroundColor Gray
Write-Host ""
...
if ($LASTEXITCODE -ne 0) { throw "V12_POSTTRAIN_HOLDOUT_COMPARE_FAILED" }

Write-Host ""
Write-Host "Kesz. Kuldd el a # SUMMARY + # BY_EVENT + # BY_LANGUAGE + # BY_VARIANT + WRONG_TRANSITIONS reszt." -ForegroundColor Cyan

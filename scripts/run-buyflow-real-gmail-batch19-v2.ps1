...
$reportPath = Join-Path $env:USERPROFILE ("Desktop\BuyFlow-REAL-GMAIL-BATCH19-V2-$stamp.json")
...
Write-Host ''
Write-Host '============================================================'
Write-Host 'BUYFLOW REAL GMAIL TARGETED BATCH19 V2'
...
# Preflight: a known-good targeted endpoint. Stop immediately if URI construction is wrong.
$preflightUri = [string]::Format('{0}?message_id={1}', $endpoint, '1a04738dd7b7f0b2')
Write-Host ('Preflight URI: ' + $preflightUri)
try {
    $preflight = Invoke-RestMethod -Uri $preflightUri -Method Get -TimeoutSec 300
    if (-not $preflight.targeted_test) { throw 'Targeted endpoint returned unexpected response.' }
    Write-Host 'Preflight: OK' -ForegroundColor Green
} catch {
    Write-Host ('HIBA PREFLIGHT: ' + $_.Exception.Message) -ForegroundColor Red
    Read-Host 'Nyomj Entert a bezarashoz'
    exit 1
}
...
foreach ($case in $cases) {
    ...
    $row = [ordered]@{
        id = $case.id; gmail_id = $case.gmail_id; source = $case.source; note = $case.note
        expected_event = $case.expected_event; expected_action = $case.expected_action
        actual_event = $null; actual_action = $null; selected_purchase_id = $null
        confidence = $null; ruleset = $null; shadow_safe = $false; write_performed = $null
        unsafe_link = $false; exact = $false; error = $null; guardrails = @(); reason = $null
    }
    try {
        $uri = [string]::Format('{0}?message_id={1}', $endpoint, $case.gmail_id)
        ...
        $row.reason = $d.reason
        $row.shadow_safe = ($r.shadow_only -eq $true -and $r.write_performed -eq $false -and $r.buyflow_result.mode -eq 'SHADOW' -and $r.buyflow_result.execution_allowed -eq $false)
        $row.unsafe_link = ($d.action -eq 'LINK_EXISTING')
        $row.exact = ($d.event_type -eq $case.expected_event -and $d.action -eq $case.expected_action -and $null -eq $d.selected_purchase_id)
...
$report = [ordered]@{ suite='REAL_GMAIL_TARGETED_BATCH19_V2'; created_at=(Get-Date).ToString('o'); total=$cases.Count; exact=$exactCount; exact_percent=$percent; event_correct=$eventCount; action_correct=$actionCount; shadow_safe=$shadowCount; ruleset_v4=$rulesetCount; unsafe_links=$unsafeCount; selected_purchase_non_null=$selectedNonNull; errors=$errorCount; gate=if($gate){'PASS'}else{'REVIEW'}; results=$results }
...
if ($failed.Count -gt 0) {
    ...
    foreach ($f in $failed) {
        if ($f.error) { Write-Host ("- {0} {1}: ERROR {2}" -f $f.id,$f.source,$f.error) }
        else { Write-Host ("- {0} {1}: expected {2}/{3}, got {4}/{5}" -f $f.id,$f.source,$f.expected_event,$f.expected_action,$f.actual_event,$f.actual_action) }
    }
    Write-Host ''
}
Read-Host 'Nyomj Entert a bezarashoz'

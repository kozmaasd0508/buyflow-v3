...
$tempRoot=Join-Path $env:TEMP ('buyflow-v13-lite-v9-' + [guid]::NewGuid().ToString('N'))
...
$checkpointDir=Join-Path $env:USERPROFILE 'Desktop\buyflow\.direct-private'
$checkpointPath=Join-Path $checkpointDir 'v13-lite-v9-real120-checkpoint.json'
...
function Start-Qwen{
  ...
  $script:serverProcess=Start-Process -FilePath 'wsl.exe' -ArgumentList $launchArgs -RedirectStandardOutput $stdout -RedirectStandardError $stderr -WindowStyle Hidden -PassThru
  try{$script:serverProcess.PriorityClass='BelowNormal'}catch{}
  $script:health=$null
  for($i=0;$i -lt 180;$i++){
    try{$script:health=Invoke-RestMethod -Uri 'http://127.0.0.1:4394/health' -Method Get -TimeoutSec 2;if($script:health.ok){break}}catch{}
    if($script:serverProcess.HasExited){break};Start-Sleep -Seconds 1
  }
  if(-not $script:health -or -not $script:health.ok){if(Test-Path $stderr){Get-Content $stderr -Tail 30};Fail 'EVENTMIND_RUNTIME_START_FAILED'}
  if($script:health.model_id -ne 'Qwen/Qwen3-8B' -or $script:health.thinking_enabled -ne $false -or $script:health.deterministic -ne $true){Fail 'EVENTMIND_RUNTIME_CONTRACT_FAILED'}
  $env:BUYFLOW_EVENTMIND_V11_ADAPTER_SHA256=[string]$script:health.adapter_sha256
}
function Refresh-GmailToken{
  $token=Invoke-RestMethod -Method Post -Uri 'https://oauth2.googleapis.com/token' -ContentType 'application/x-www-form-urlencoded' -Body @{client_id=$clientId;client_secret=$clientSecret;refresh_token=$refreshToken;grant_type='refresh_token'} -TimeoutSec 30
...
New-Item -ItemType Directory -Force -Path $tempRoot,$checkpointDir|Out-Null
try{
  Write-Host ''
  Write-Host '==============================================================' -ForegroundColor Cyan
  Write-Host 'BUYFLOW EVENTMIND V13-LITE - REAL120 DIRECT V9' -ForegroundColor Cyan
  Write-Host 'KIMELO + FOLYTATHATO + AUTOMATIKUS QWEN UJRAINDITAS' -ForegroundColor Cyan
  Write-Host 'NINCS TESTLAB / NINCS GITHUB RUNNER' -ForegroundColor Cyan
  Write-Host '==============================================================' -ForegroundColor Cyan
  Write-Host 'Gmail csak olvasas. BuyFlow production iras: 0.' -ForegroundColor Green
  Write-Host ''
  ...
  Write-Host '[1/4] V13-lite kod letoltese...' -ForegroundColor Yellow
  ...
  if(-not $refreshToken){
    ...
    if($tokenData){$parsed=if($tokenData -is [string]){try{$tokenData|ConvertFrom-Json}catch{$null}}else{$tokenData};if($parsed){$refreshToken=[string](Get-PropertyValue $parsed @('refresh_token','refreshToken'))}}
  }
  if(-not $clientId -or -not $clientSecret -or -not $refreshToken){Fail 'GMAIL_REFRESH_CREDENTIALS_MISSING'}
  Refresh-GmailToken

  Write-Host '[3/4] Kimeletes helyi Qwen inditasa...' -ForegroundColor Yellow
  $server=Join-Path $codeRoot 'scripts\eventmind-v11-runtime.py'
  $wslProject=Convert-ToWslPath $codeRoot;$wslServer=Convert-ToWslPath $server
  ...
  $env:BUYFLOW_EVENTMIND_INTER_CASE_DELAY_MS='1000'

  Write-Host '[4/4] 120 valodi Gmail level -> V13-lite...' -ForegroundColor Yellow
  Write-Host '1 mp piheno levelenkent. Timeout eseten Qwen ujraindul es ugyanazt a levelet ujraprobalja.' -ForegroundColor DarkGray
  $runtimeRestarts=0
  $maxRuntimeRestarts=20
  while($true){
    Refresh-GmailToken
    Start-Qwen
    Push-Location (Join-Path $codeRoot 'apps\api')
    try{
      & npm.cmd exec -- tsx 'src\scripts\eventmind-v13-real-gmail-dev120-resumable.ts' $IdFile $reportPath $checkpointPath
      $exit=$LASTEXITCODE
    }finally{Pop-Location}

    if($exit -eq 75){
      $runtimeRestarts+=1
      Write-Host ("Qwen timeout -> tiszta ujrainditas ($runtimeRestarts/$maxRuntimeRestarts)...") -ForegroundColor Yellow
      if($runtimeRestarts -gt $maxRuntimeRestarts){Fail 'TOO_MANY_RUNTIME_RESTARTS'}
      Stop-Qwen
      continue
    }
    if($exit -eq 76){
      Write-Host 'Gmail token frissites -> folytatas...' -ForegroundColor Yellow
      Stop-Qwen
      continue
    }
    if($exit -eq 0 -or $exit -eq 2){break}
    Fail "V13_LITE_RESUMABLE_FATAL_EXIT_$exit"
  }
  ...
  Write-Host ''
  Write-Host ('V13-LITE DIRECT V9 BLOCKED/FAIL - ' + $_.Exception.Message) -ForegroundColor Red
  Write-Host 'Semmi nincs elesitve. Gmail/BuyFlow production modositas nem tortent.' -ForegroundColor Yellow
  Write-Host ('Checkpoint megtartva: ' + $checkpointPath) -ForegroundColor DarkGray
  ...
  Remove-Item Env:BUYFLOW_EVENTMIND_INTER_CASE_DELAY_MS -ErrorAction SilentlyContinue
  Stop-Qwen
  ...
  if($repoRoot -and (Test-Path -LiteralPath $codeRoot)){try{Cmd-Git $repoRoot ('worktree remove --force "' + $codeRoot + '"')|Out-Null}catch{};try{Cmd-Git $repoRoot 'worktree prune'|Out-Null}catch{}}

...
Write-Host 'BUYFLOW GPT-OSS 20B - H1 DIAGNOSTIC 5 V3' -ForegroundColor Cyan
Write-Host 'ASCII-safe script source for Windows PowerShell 5.1.' -ForegroundColor Yellow
Write-Host 'Bundle is decoded explicitly as UTF-8.' -ForegroundColor Yellow
...
# ASCII-safe UTF-8 sanity check using Unicode code points, not non-ASCII script literals.
...
if(-not $h1012 -or [string]$h1012.subject -ne $expectedSubject){
  throw ('UTF8_DECODE_SANITY_FAILED:H1-012:'+([string]$h1012.subject))
}
...
foreach($caseId in $caseIds){
  ...
  try {
    ...
    $sw.Stop()
    $message=Get-Prop $r 'message'
    $content=[string](Get-Prop $message 'content')
    $thinking=[string](Get-Prop $message 'thinking')
    $row.http_ok=$true
    ...
    Write-Host ('HTTP: OK | '+$sw.ElapsedMilliseconds+' ms | content chars='+$content.Length+' | thinking chars='+$thinking.Length+' | done='+$row.done_reason) -ForegroundColor Green
...
$outPath=Join-Path $bundle.Directory.FullName 'real60-h1-gpt-oss20b-diagnostic5-v3-private-local.json'
$result=[ordered]@{
  diagnostic='buyflow-gpt-oss20b-h1-diagnostic5-v3'

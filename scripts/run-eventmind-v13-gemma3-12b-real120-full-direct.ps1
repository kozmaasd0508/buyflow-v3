...
$codeCommit='f779acc64dabdfea5b476e592da5c6490c736a77'
...
$temp=Join-Path $env:TEMP ('buyflow-gemma3-real120-full-base-' + [guid]::NewGuid().ToString('N') + '.ps1')
$patched=Join-Path $env:TEMP ('buyflow-gemma3-real120-full-run-' + [guid]::NewGuid().ToString('N') + '.ps1')
...
try {
  ...
  Write-Host 'BUYFLOW EVENTMIND - REAL120 GEMMA 3 12B FULL EMAIL TEST' -ForegroundColor Cyan
  Write-Host 'NO CHUNKS / NO FINAL JUDGE / V4 PROMPT / JSON OUTPUT' -ForegroundColor Cyan
  ...
  $parserGood='Write-Host ("--- SEGMENT ${segment}: Gmail token frissites + Gemma 3 12B FULL EMAIL ---") -ForegroundColor Cyan'
  ...
  $text=$text.Replace('BuyFlow-EVENTMIND-REAL120-CHUNK-JUDGE-CHECKPOINT.json','BuyFlow-EVENTMIND-REAL120-GEMMA3-12B-FULL-V4-CHECKPOINT.json')
  $text=$text.Replace('BuyFlow-EVENTMIND-REAL120-CHUNK-JUDGE-$stamp.json','BuyFlow-EVENTMIND-REAL120-GEMMA3-12B-FULL-V4-$stamp.json')
  $text=$text.Replace('BuyFlow-EVENTMIND-REAL120-CHUNK-JUDGE-GPU-$stamp.jsonl','BuyFlow-EVENTMIND-REAL120-GEMMA3-12B-FULL-V4-RUNTIME-$stamp.jsonl')
...
'@
  ...
  $text=NeedReplace $text "& npm.cmd exec --yes -- tsx 'src\scripts\eventmind-v13-real-gmail-chunk-judge-dev120.ts' `$IdFile `$checkpointPath 15" "& npm.cmd exec --yes -- tsx 'src\scripts\eventmind-v13-real-gmail-full-dev120.ts' `$IdFile `$checkpointPath 15" 'EXPECTED_TS_RUNNER_CALL_NOT_FOUND'

  $text=$text.Replace('BUYFLOW EVENTMIND V13-LITE - REAL120 CHUNK + FINAL JUDGE','BUYFLOW EVENTMIND - REAL120 GEMMA 3 12B FULL EMAIL')
  $text=$text.Replace('DIRECT / CHECKPOINT / QWEN RESTART 15 LEVELENKENT','NO CHUNKS / NO FINAL JUDGE / CHECKPOINT / JSON OUTPUT')
  $text=$text.Replace("Write-Host 'Qwen runtime: READY' -ForegroundColor Green","Write-Host ('Gemma FULL runtime: READY | digest ' + [string]`$health.adapter_sha256) -ForegroundColor Green")
  ...
  $text=$text.Replace("Write-Host ('GPU log: ' + `$diagLogPath) -ForegroundColor DarkGray","Write-Host 'Runtime: Ollama gemma3:12b | FULL semantic email | V4 prompt | JSON mode' -ForegroundColor DarkGray")
  $text=$text.Replace('REAL120 CHUNK + FINAL JUDGE: COMPLETE','REAL120 GEMMA 3 12B FULL EMAIL: COMPLETE')

...
const TARGETS=['H2-004','H2-005','H2-006','H2-011','H2-012','H2-016','H2-037','H2-039','H2-064','H2-073','H2-079','H2-091','H2-095'];
const EXPECTED={
  ...
  'H2-005':{event_type:'SHIPMENT_CREATED',perspective:'merchant_outbound',order_id:null,tracking_id:null,link_status:'not_applicable'},
  'H2-006':{event_type:'SHIPMENT_CREATED',perspective:'merchant_outbound',order_id:null,tracking_id:null,link_status:'not_applicable'},
  'H2-011':{event_type:'SHIPMENT_CREATED',perspective:'merchant_outbound',order_id:null,tracking_id:null,link_status:'not_applicable'},
  'H2-012':{event_type:'SHIPMENT_CREATED',perspective:'merchant_outbound',order_id:null,tracking_id:null,link_status:'not_applicable'},
...
- IMPORTANT: merchant_outbound is only the perspective. It does NOT imply event_type OTHER. After choosing merchant_outbound, still classify the current logistics event.
- For merchant_outbound, if a carrier pickup/collection request has been recorded, accepted or scheduled but the parcel has NOT yet been physically collected, use SHIPMENT_CREATED.
- For merchant_outbound, if the carrier has physically collected the parcel from the mailbox owner, use SHIPPED unless a later carrier-network movement state is explicitly current.
...
- SHIPMENT_CREATED = shipment/tracking/pre-advice exists, parcel is packed and waiting for carrier, OR a carrier pickup/collection booking has been recorded/accepted/scheduled but physical collection has not happened.
...
- merchant_outbound pickup booking/acceptance/schedule without physical collection => SHIPMENT_CREATED + null IDs + not_applicable.
- merchant_outbound physical carrier collection => SHIPPED + null IDs + not_applicable, unless a later carrier state is current.
...
async function callModel(apiKey,emailText){const retry=[1000,2500,6000];for(let attempt=0;;attempt++){const res=await fetch(API_URL,{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:MODEL,instructions:SYSTEM,input:`Email:\n\n${emailText}`,reasoning:{effort:'low'},text:{verbosity:'low',format:{type:'json_schema',name:'buyflow_email_classification_v31',strict:true,schema:SCHEMA}},max_output_tokens:512,store:false})});if(res.ok){const data=await res.json();const text=extractOutputText(data);if(!text)throw new Error('OPENAI_NO_OUTPUT');return{prediction:enforcePrediction(JSON.parse(text)),usage:data.usage??{}};}const detail=await res.text().catch(()=>'');if([408,429,500,502,503,504].includes(res.status)&&attempt<retry.length){await sleep(retry[attempt]);continue;}throw new Error(`OPENAI_HTTP_${res.status}:${detail.slice(0,250)}`);}}

async function main(){
  const bundlePath=process.argv[2];
  const outPath=process.argv[3]||path.join(path.dirname(bundlePath),'real100-h2-prompt-v31-luna-regression13.json');
  ...
  console.log('==============================================================');
  console.log('BUYFLOW H2 SPENT-DATA REGRESSION - LUNA PROMPT V3.1');
  console.log('13 real emails | merchant-outbound fix + regression controls');
  console.log('H2 is tuning data, NOT blind accuracy. No Gmail. O3 NOT USED.');
  console.log('==============================================================');
  ...
  for(let i=0;i<TARGETS.length;i++){
    ...
    try{
      ...
      usage.input_tokens+=Number(r.usage?.input_tokens??0);usage.output_tokens+=Number(r.usage?.output_tokens??0);usage.reasoning_tokens+=Number(r.usage?.output_tokens_details?.reasoning_tokens??0);
  ...
  const summary={benchmark:'buyflow-h2-spent-prompt-v31-luna-regression13',created_at:new Date().toISOString(),model:MODEL,prompt_version:'merchant-outbound-linking-v3.1',mail_lens_normalizer:EXPECTED_NORMALIZER,selection_sha256:bundle.selection_sha256,total:TARGETS.length,exact,exact_pct:Number((exact/TARGETS.length*100).toFixed(1)),technical_errors:errors,usage,safety:{gmail_calls:0,buyflow_writes:0,production_off:true,blind_o3_used:false,openai_store:false},warning:'H2 is spent/tuning data; this is regression evidence only, not blind production accuracy.',rows};
  await writeFile(outPath,JSON.stringify(summary,null,2)+'\n','utf8');
  console.log('');console.log(`V3.1 regression exact: ${exact}/${TARGETS.length} = ${summary.exact_pct}%`);console.log(`Technical errors: ${errors}`);console.log(`Result: ${outPath}`);console.log('==============================================================');
}
main().catch(e=>{console.error(`H2_V31_FATAL: ${e instanceof Error?e.message:String(e)}`);process.exitCode=1;});

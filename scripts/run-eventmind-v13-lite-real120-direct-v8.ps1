...
$tempRoot=Join-Path $env:TEMP ('buyflow-v13-lite-v8-' + [guid]::NewGuid().ToString('N'))
...
try{
  ...
  Write-Host 'BUYFLOW EVENTMIND V13-LITE - REAL120 DIRECT V8' -ForegroundColor Cyan
  Write-Host 'REGI MODSZER - NINCS TESTLAB - NINCS GITHUB RUNNER' -ForegroundColor Cyan
  Write-Host '==============================================================' -ForegroundColor Cyan
  ...
  Write-Host '[1/4] V13-lite branch letoltese...' -ForegroundColor Yellow
  ...
  $nodeSource=Join-Path $repoRoot 'node_modules';$nodeTarget=Join-Path $codeRoot 'node_modules'
  ...
  Write-Host '[3/4] Helyi Qwen inditasa...' -ForegroundColor Yellow
  ...
  try{& npm.cmd exec -- tsx 'src\scripts\eventmind-v13-real-gmail-dev120.ts' $IdFile $reportPath;$exit=$LASTEXITCODE}finally{Pop-Location}
  ...
  Write-Host ''
  Write-Host ('V13-LITE DIRECT V8 BLOCKED/FAIL - ' + $_.Exception.Message) -ForegroundColor Red

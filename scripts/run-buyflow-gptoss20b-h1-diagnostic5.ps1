...
Write-Host 'BUYFLOW GPT-OSS 20B - H1 DIAGNOSTIC 5' -ForegroundColor Cyan
Write-Host 'Cases: H1-003, H1-008, H1-012, H1-035, H1-052' -ForegroundColor Yellow
Write-Host 'Same MailLens v1.1 + frozen Prompt V2.' -ForegroundColor Green
...
foreach($caseId in $caseIds){
  ...
  try {
    ...
    Write-Host ('HTTP: OK | '+$sw.ElapsedMilliseconds+' ms | content chars='+$content.Length+' | thinking chars='+$thinking.Length) -ForegroundColor Green
...
$outPath=Join-Path $bundle.Directory.FullName 'real60-h1-gpt-oss20b-diagnostic5-private-local.json'
$result=[ordered]@{
  diagnostic='buyflow-gpt-oss20b-h1-diagnostic5'
...
Write-Host 'Upload this JSON or paste the terminal output here.' -ForegroundColor Yellow

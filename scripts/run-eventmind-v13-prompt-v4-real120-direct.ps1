...
try {
  ...
  Write-Host '==============================================================' -ForegroundColor Cyan
  Write-Host ('Code commit: ' + $codeCommit) -ForegroundColor Green
  ...
  $text=$text.Replace("`$branch='codex/buyflow-testlab-v1'", "`$branch='$codeCommit'")
  $text=$text.Replace('Write-Host ("--- SEGMENT $segment: Gmail token frissites + tiszta Qwen ---") -ForegroundColor Cyan', 'Write-Host ("--- SEGMENT ${segment}: Gmail token frissites + tiszta Qwen ---") -ForegroundColor Cyan')

...
$generatorCommit='f1a05c938c04b100362e0849c4435a406bf8caec'
$generatorUrl="https://raw.githubusercontent.com/kozmaasd0508/buyflow-v3/$generatorCommit/apps/api/src/scripts/buyflow-real100-h2-blind-maillens-v11.ts"
...
$tempRoot=Join-Path $env:TEMP ('buyflow-real100-h2-' + [guid]::NewGuid().ToString('N'))
...
try {
  ...
  Write-Host 'BUYFLOW REAL100 H2 - FRESH BLIND / MAILLENS v1.1' -ForegroundColor Cyan
  Write-Host '100 real Gmail purchase emails. REAL120 + H1 excluded.' -ForegroundColor Green
  Write-Host 'NO Luna/OpenAI calls in this step. Gmail GET only.' -ForegroundColor Green
  ...
  $outDir=Join-Path $preferredBase ('buyflow-real100-h2-'+$stamp)
  ...
  Write-Host '[5/5] Select and freeze 100 unseen H2 emails...' -ForegroundColor Yellow
  $runnerPath=Join-Path $worktree 'apps\api\src\scripts\__buyflow-real100-h2-temp.ts'
  ...
  if($exit -ne 0){Fail "REAL100_H2_EXIT_$exit"}
  ...
  Write-Host 'Keep the private IDs file locally. Do not run a model yet.' -ForegroundColor Yellow
}
finally {

...
begin
  ...
  set
    current_state = case
      ...
      when current_state = 'ready_for_pickup' then 'ready_for_pickup'
      when p_status = 'ready_for_pickup' then 'ready_for_pickup'
      when current_state in ('processing', 'ordered', 'paid', 'in_transit') then 'in_transit'
      else current_state
    end,
    shipped_at = case
